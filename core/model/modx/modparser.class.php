...
class modParser {
    ...
    public $timerDepth = 0;
    public $timerId = 0;
    public $currentParent = array( 0 => 0 );
    /**
     * hack #1 ends
     */
    /**
    ...
    public function processTag($tag, $processUncacheable = true) {
        ...
        if ($this->modx->getOption('parser_timer', null, false)) {
          $currentTimerId = $this->timerId;
          $currentTimeDepth = $this->timerDepth;
          $this->currentParent[$currentTimeDepth+1] = $currentTimerId;
          $this->timer['timer'][$currentTimerId]=array(
              'children' => array(),
              'childrentotal' => 0,
              'timefiltered'  => 0
          );
        ...
        if ($this->modx->getOption('parser_timer', null, false)) {
          ...
          $timer['type'] = 'processTag';
          $timer['objectName'] = $tagName;
          $timer['objectParams']= $innerTag;
          $timer['time'] = rtrim(sprintf('%.20F', $endTime - $startTime), '0');
          $timer['timerId'] = $currentTimerId;
          //$timer['tagName'] = $tagName;
          //$timer['outerTag'] = $outerTag;
          //$timer['innerTag'] = $innerTag;
          //$timer['elementOutput'] = $elementOutput;

          //$timer['elementOutput'] = $output;
          $timer['timerDepth'] = $currentTimeDepth;
          $this->timer['timer'][$currentTimerId]['timer'] = $timer;
          if ($currentTimeDepth)
          {
            $this->timer['timer'][$this->currentParent[$currentTimeDepth]]['children'][$currentTimerId] = $timer;
            $this->timer['timer'][$this->currentParent[$currentTimeDepth]]['childrentotal'] += $timer['time'];
          }
/*            $this->timer['time'][$startTime] = $timer;
          $this->timer['timelength'][$timer['time']] = $timer;
          $this->timer['timedepth'][$currentTimeDepth][] = $timer;
*/
    ...
    public function getTimer() {
        $output = array();
        if ($this->modx->getOption('parser_timer', null, false)) {
          switch($this->modx->getOption('parser_print', null, false))
          {
            case 'sortbyName':
              foreach ($this->timer['timer'] as $id=>$v) {
                $this->timer['timer'][$id]['timefiltered']
                = rtrim(sprintf('%.20F',
            ...
            $total = array();
            foreach ($this->timer['timer'] as $id=>$v) {
              $total[] = $this->timer['timer'][$id]['timefiltered']
              = rtrim(sprintf('%.20F',
              round(  $this->timer['timer'][$id]['timer']['time']
              - $this->timer['timer'][$id]['childrentotal'],8)),
              '0') ;
              //$total[] = $v['timer']['time'];
              //$total[] = $v['timefiltered'];
              $subtotal=array();
              foreach ($v['children'] as $subid=>$subv)
              {
                $subtotal[] = $subv['time'];
              }
              array_multisort($subtotal, SORT_DESC, SORT_NUMERIC, $this->timer['timer'][$id]['children']);

            }
            array_multisort($total, SORT_DESC, SORT_NUMERIC, $this->timer['timer']);
            $output = array(
